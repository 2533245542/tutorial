mistudio_filter_otu_aggregate = function(phyobj, filter_rank){
	...
	get_aggregate = function(otu, tax, filter_rank){
		get_otu_filter = function(otu, tax, filter_rank){
			browser()			
			otu_filter = t(aggregate(otu, list(tax[, filter_rank, drop = TRUE]), FUN = sum)[-1]) # aggregate based on unique tax[filter_rank]
																			   # then sum up all rows in a group, and [-1] to
																			   # remove the group variable column
			colnames(otu_filter) = paste0(filter_rank, unlist(aggregate(otu, list(tax[, filter_rank, drop = TRUE]), FUN = sum)[1]))
			# colnames(otu_filter) = aggregate(otu, list(tax[, filter_rank, drop = TRUE]), FUN = sum)[1]
