...
mistudio_milineagec = function(
	...
	# 规定：这个选的一定要是每个ID group都有的，选numeric的时候可以选整个column但是categorical的话只能选subcategroy
	.Rarefy = function (otu.tab) 
	{	      
	  depth = min(rowSums(otu.tab))
	  otu.tab <- as.matrix(otu.tab)
	...
	get_OTU.base = function(phyobj, milineagec_function, milineagec_id, milineagec_timebase){
	  # do not do varify in this function
	  get_OTU = function(phyobj, milineagec_function){
	    otu = otu_table(phyobj)@.Data
	    tax = tax_table(phyobj)@.Data     
	  ...
	  # rarefy the otu base. Original phyobj is needed to make sure the dimension of otu is matched before .Rarefy()
	  get_OTU2 = function(phyobj, milineagec_function, OTU_without_base, OTU_base){
	    otu = otu_table(phyobj)@.Data
	    tax = tax_table(phyobj)@.Data     
	    .Rarefy = function (otu.tab) 
	    {	      
	    ...
	    if(milineagec_function == "QCAT_GEE.Cluster"){
	      OTU_without_base = .Rarefy(OTU_without_base)  
	    }	    
	    return(OTU_without_base)
	  }
	  ...
	  # order otu, sample by ID
	  order_id = order(sample[, milineagec_id, drop = TRUE])
	  sample = sample[order_id, ]
	  OTU = OTU[order_id, ]
	  ...
	  # get otu, sample, id without base
	  if(value == column){ # numeric time base selected
	    # value = max(sample[, column, drop = TRUE])
	    value = min(sample[, column, drop = TRUE])
	  } else { # categorical time base selected(remember that no whole categorical is allowed to be selected for time base)
	    value = value 
	  }
	  base.idx = which(sample[,column]==value)
	  OTU_without_base = OTU[-base.idx,]
	  sample_without_base = sample[-base.idx, ]
	  id_without_base = sample[-base.idx, milineagec_id]

	  # to find the base, we need the to varify the candidate matrix first	  
	  OTU_base_rarefy = .Rarefy(OTU[base.idx, ])
	  OTU_inserted = OTU
	  for(i in 1:nrow(OTU_base_rarefy)){
	  	OTU_inserted[base.idx[i], ] = OTU_base_rarefy[i, ]
	  }

	  by_otu = by(OTU_inserted, sample[, milineagec_id, drop = TRUE], function(x) return(x)) # note that OTU is transposed here so the row dimension allighs with sample's row
	  ...
	  otubase_helper = function(otu, sample, value, column){
	    ...
	    return(
	      do.call(rbind, replicate(nrow(otu)-1, otu[row_index, , drop = FALSE], simplify = FALSE))
	    )
	  }
	  list_of_mat = mapply(FUN = otubase_helper, by_otu, by_sample, MoreArgs = list(value = value,  column = column), SIMPLIFY = FALSE) # worth noticing that each by_otu and by_sample have the same row names
	  OTU_base = do.call(rbind, list_of_mat) # transpose OTU to make row as otu to be acceptable by miLineage
	  OTU_base = as.matrix(OTU_base)
	  
	  OTU_base = OTU_base/rowSums(OTU_base)	 
	  OTU_without_base = get_OTU2(phyobj, milineagec_function, OTU_without_base)

	  return(
	    list(base.idx = base.idx, id_without_base=id_without_base, OTU_without_base=OTU_without_base, OTU_base=OTU_base)
	  )
	}
  ...
  if(milineagec_function == "QCAT.Cluster"){		
		if(is.null(milineagec_timebase)){
			return(QCAT.Cluster(ID = ID, OTU = OTU, OTU.base=NULL, X = X, X.index = X.index, Tax=Tax, min.depth=min.depth, perm.type=perm.type, n.perm=n.perm, fdr.alpha=fdr.alpha, test=test))	
		} else{
			base.idx = OTU.base[[1]]
			X = X[-base.idx, , drop = FALSE]
			ID = OTU.base[[2]]
			OTU = OTU.base[[3]]
			OTU.base = OTU.base[[4]]		
		...
		if(is.null(milineagec_timebase)){
			return(QCAT_GEE.Cluster(ID = ID, OTU = OTU, OTU.base=NULL, X = X, X.index = X.index, Z = Z, Z.index = Z.index, Tax=Tax, min.depth=min.depth, perm.type=perm.type, n.perm=n.perm, fdr.alpha=fdr.alpha, test=test))	
		} else{
			base.idx = OTU.base[[1]]
			X = X[-base.idx, , drop = FALSE]
			Z = Z[-base.idx, , drop = FALSE]
			ID = OTU.base[[2]]
			OTU = OTU.base[[3]]
			OTU.base = OTU.base[[4]]	
